...
- отключение постоянных уведомлений о выключенном Брандмауэре Windows
- `NVIDIA App`
...
- собственное приложение `DeepCool Companion` для `AK400 Digital`
...
- включение пункта `Завершить задачу` в панели задач
...
- сборка и запуск `DeepCool Companion`
...
- [Cooling.psm1](c:\Users\danil\Desktop\FirstSetup-Windows\Modules\Cooling.psm1) - DeepCool diagnostics и companion-интеграция
- [DeepCoolCompanion.csproj](c:\Users\danil\Desktop\FirstSetup-Windows\Tools\DeepCoolCompanion\DeepCoolCompanion.csproj) - собственное tray-приложение для `AK400 Digital`
...
- если не найден `Microsoft Store`, скрипт пробует восстановление через `wsreset -i`
- требуется установленный `winget`
- для сборки `DeepCool Companion` нужен установленный `.NET SDK`
...
Для `AK400 Digital` проект использует только собственный `DeepCool Companion`.

Что делает:
- работает в трее как один процесс
- использует прямой `HID`-доступ к устройству `VID_3633 / PID_0001`
- читает температуру и загрузку CPU через `LibreHardwareMonitor`
- умеет режимы `Temperature`, `Utilization`, `Automatic`
- умеет `Celsius/Fahrenheit`
- может запускаться вместе с Windows
- при старте останавливает `deepcool-digital`, если он остался в системе, чтобы не было конфликта

Как запустить из меню:
- `Оборудование и драйверы -> Собрать и установить DeepCool Companion`
- `Оборудование и драйверы -> Запустить DeepCool Companion`

Ручная сборка:

```powershell
dotnet publish .\Tools\DeepCoolCompanion\DeepCoolCompanion.csproj -c Release -r win-x64 --self-contained false -o .\Tools\DeepCoolCompanion\publish
```

Ручной запуск:

```powershell
.\Tools\DeepCoolCompanion\publish\DeepCoolCompanion.exe
```
